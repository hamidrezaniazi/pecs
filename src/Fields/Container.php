...
class Container extends AbstractEcsField
{
    ...
    protected function body(): Collection
    {
        return collect([
            'cpu' => [
                'usage' => $this->cpuUsage?->scale(),
            ],
            'disk' => [
                'read' => [
                    'bytes' => $this->diskReadBytes,
                ],
                'write' => [
                    'bytes' => $this->diskWriteBytes,
                ],
            ],
            'id' => $this->id,
            'image' => [
                'hash' => [
                    'all' => $this->imageHashAll?->toArray(),
                ],
                'name' => $this->imageName,
                'tag' => $this->imageTag?->toArray(),
            ],
            'labels' => $this->labels?->toArray(),
            'memory' => [
                'usage' => $this->memoryUsage?->scale(),
            ],
            'name' => $this->name,
            'network' => [
                'egress' => [
                    'bytes' => $this->networkEgressBytes,
                ],
                'ingress' => [
                    'bytes' => $this->networkIngressBytes,
                ],
            ],
